        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="mt-1">Users
                        <a wire:navigate href="{{url('admin/user/create')}}" class="btn btn-primary rounded text-white float-end" data-bs-toggle="modal" data-bs-target="#addBrandModal">Add User</a>
                    </h3>
                </div>
                            ...
                            <tbody>
                                @forelse ($users as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->name}}</td>
                                        <td>{{$item->email}}</td>
                                        <td><span class="{{$item->role_as == '0'?'bg-info':'bg-primary'}} text-light px-2 py-1 rounded">{{$item->role_as == '0'?'User':'Admin'}}</span></td>
                                        <td>
                                            <a wire:navigate href="{{url('admin/user/'.$item->id.'/edit')}}" class="btn mx-2 btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editBrandModal">
                                                <small class="fa-solid fa-pen"></small>
                                                <small>Edit</small>
                                            </a>
                                            <a wire:click="deleteUser({{$item->id}})" class="text-white btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deletemodal">
